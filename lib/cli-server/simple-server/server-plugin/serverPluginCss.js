...
async function processCss(filePath) {
  // path.resolve(process.cwd(), 'node_modules', ctx.path.replace(thirdPartModuleRE, ''))
  const thirdPartModuleRE = /^\/@module\//
  let finalPath
  if (thirdPartModuleRE.test(filePath)) {
    finalPath = path.resolve(process.cwd(), 'node_modules', filePath.replace(thirdPartModuleRE, ''))
  } else {
    finalPath = path.resolve(process.cwd(), filePath.slice(1))
  }
  ...
  const {css} = await transformCss(code, lang)
  return css
}
...
async function transformCss (source, lang) {
  if (!lang || lang === 'css') return {css: source}
  let result = {}
  if (lang === 'less') {
    // TODO less未支持依赖文件加载
